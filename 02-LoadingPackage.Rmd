...
Motus **users** can install the latest stable versions of the R packages using the following code. As with all R packages, you only need to install the packages once, and after installation, you need to load each package (using library() or require()) each time you open a new R session. 
...
install_github("MotusWTS/motusClient@motusserver")   
library(motusClient)

# install motus for data manipulation, visualization and analysis
install_github("MotusWTS/motus@staging")  
library(motus)
