...
# library(motusClient)  ## no longer necessary?
library(motus)    
...
df.projRecvs <- tbl.recvDeps %>%
                ...
                mutate(tsStart = as_datetime(tsStart, tz = "UTC", origin = "1970-01-01"),
                       ...
                       deployID = as.factor(deployID)) ## why not make these factors to begin with?
...
Looking at the 'isMobile' column for the three receiver deployments that are missing latitude and longitude information, it is evident that these are mobile receivers that do not have a fixed position, and so it is not possible to assign a latitude and longitude (these fields will remain NA). 
...
ggplot(df.projRecvs.long, 
       aes(y = deviceID, x = ts, colour = deployID)) +
  geom_line(lwd=3) + 
...
```{r}
ggplot(df.projRecvs.long, 
       aes(y = deviceID, x = yday(ts), colour = deployID)) +
  geom_line(lwd=3) + 
...
ggplot(na.lakes, aes(long, lat))+ 
  ...
  xlab("") + ylab("") + 
  theme_bw() + 
  geom_point(data = df.recvDeps, aes(longitude, latitude, colour = as.logical(projectID == 38)), 
             cex = 0.8, pch = 4)+
...
ggplot(na.lakes, aes(long, lat))+ 
  ...
  geom_point(data = filter(df.projRecvs, 
             ...
             aes(longitude, latitude, colour = deviceID), cex = 2, pch = 1)+
