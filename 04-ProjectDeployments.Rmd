...
The number of rows in the 'df.tags' database is equivalent to the number of tags registered to the James Bay Shorebird Project:
...
Recall we have access to just a small sample of these tags in the sample dataset.
...
## Species metadata
tbl.species <- tbl(t, "species") 
df.species <- tbl.species %>%
...
Receiver metadata errors or ommissions that you find in your .motus file can only be fixed for receivers registered to your own project. All users are encouraged to enter complete and accurate receiver metadata for the benefit of the entire network. 
If you anticipate needing specific information on receiver or antenna deployments for your project, please consider using the Motus listserve to request that other registered users record the receiver deployment details you will need; be specific about the exact receiver deployment details you are interested in, and when and where in the network your tags will be deployed and potentially detected. 
...
tbl.projRecvs <- tbl(t, "recvDeps") 
df.projRecvs <- tbl.projRecvs %>%
                filter(projectID == proj.num) %>%
                collect() %>%
                as.data.frame() %>%
                mutate(tsStart = as_datetime(tsStart, tz = "UTC", origin = "1970-01-01"),
...
There are `r nrow(df.projRecvs)` receiver deployments registered to the sample project. Four deployments are missing latitude and longitude, and two deployments are missing end dates, which suggests those receivers are still deployed. 
...
Looking at the 'name' and 'isMobile' columns for the four receiver deployments that are missing latitude and longitude information, it is evident that these are mobile receivers which do not have a fixed position. 

The parameter'isMobile' = 0 for deployIDs 3813 and 3814, despite being named as mobile receivers; the classification of these receiver deployments should be changed to 'isMobile' = 1 online at <https://motus.org> under receiver deployments for this project.

### 2. Timing of project receiver deployments

The timing of deployments can be displayed graphically; horizontal line(s) in the following plot show the time span for each receiver (deviceID) deployment registered to the James Bay Shorebird Project. Different deployments of the same receiver should not overlap in time:
