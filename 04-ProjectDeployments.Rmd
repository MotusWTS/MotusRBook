...
df.tags <- readRDS("./df.tags.RDS") %>%
  filter(projectID == 38) 

```

The number of rows in the 'df.tags' database is equivalent to the number of tags registered on Motus to the James Bay Shorebird Project. There are xx tags registered to the project (recall we are using just a subset of this dataset):
...
df.tagDeps <- readRDS("./df.tagDeps.RDS") %>%
  filter(projectID == 38) 
