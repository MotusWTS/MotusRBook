...
install_github("jbrzusto/motus@staging")   ## the last stable version
library(motus)
```
...
Your project's .motus file is available for download at <https://motus.org/data/downloads> under your project profile. Once downloaded, **make sure that you have the appropriate packages installed and loaded in R as outlined in Chapter 2 - Loading the Motus R Package**. 

Once all required packages are loaded, you can use the following code to load your project data, or the sample data from the James Bay Shorebird Project ('sampledb.motus'; see 'Sample Dataset' section of Chapter 1 - Introduction):
...
### Convert a virtual table to a flat dataframe
...
### Export your "flat" dataframe to a .csv file
...
file.name <- "data/sampledb.motus"    ## replace with the full location of your project data
t <- dplyr::src_sqlite(file.name)
df <- tbl(t, "alltags")
...
```{r tagMeta, eval = FALSE, message = FALSE, warning = FALSE}
...
The following tables and associated fields are included in each .motus file:
