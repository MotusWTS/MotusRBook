...
**1. motusTagID 16038:** 
- The detections around Lake Michigan have a runLen < 2, we can more closely examine what these look like using `r filter(df.alltags.sub, motusTagID == 16038, recvLon < -85), eval = FALSE`, and we see that the detections near Lake Michigan occurred on August 21 and have a run length of only 2. Because we don't have a strong reason to trust these to be true detections, we can safely drop these observations from our data. 

To do so, we create a dataframe which contains the projectID, motusTagID, ambigID and runID of detections we would like to remove. We do not remove them from the data yet:
...
Now re-run the above plot, and we now have just two individuals with movements between towers. Recall that the primary purpose of this data was to look at movement behaviour at the wintering site, so broad-scale movements are primarily bonus information:
...
Rate of movement between sites can be determined using the siteTrans function in the motus R package (see \@ref(#appendixC).  
