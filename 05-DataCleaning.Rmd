...
```
We can check that the filter was created using listRunsFilters:
...
We already have our 'df.runs' dataframe, so all we need to do is add a probability of 0 to those detections, and subset to motusTagID, runID and probability:

```{r addProb0}

df.runs <- mutate(df.runs, probability = 0) %>%
              select(runID, motusTagID, probability)
...
writeRunsFilter(sql.motus, "filtAmbigFalsePos", motusProjID = 176, df = df.runs)
...
writeRunsFilter(sql.motus, "filtAmbigFalsePos", motusProjID = 176, df = df.runs, delete = TRUE)
...
Finally, we can apply the filter to our data. Note that applying the filter simply assigns each run of detections with the probability you've assigned in the steps above. To also filter the data by those probabilities, you need to use the `p.min` statement. Here we drop anything with probability less than 0.1, although because we've assigned all runIDs that we want to drop a 0, there are currently only 0 and 1 probabilities in our data:
