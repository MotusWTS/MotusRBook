...
df.alltags.sub <- left_join(df.alltags, df.block.all, by = c("runID", "motusTagID")) %>%
  mutate(probability = ifelse(is.na(probability), 1, 0)) %>% # assign a probability of 1 to the records that will not be filtered
  filter(probability > 0)

```

Now save the local data frame as an RDS file, for use in the next chapter. Recall from \@ref(#exportDetections) that the RDS structure preserves the R data structure, such as time stamps. The other benefit of saving to RDS is you have the output from a given workflow saved as a flat file, and you won't need to go back through that workflow to reproduce the data.
...
As an alternative to saving your data as an RDS file, the Motus R package offers functionalities to create custom filters for your data. The filters can be used to 1) assign probabilities to detections (runs), and 2) filter detections based on those probabilities each time you load your .motus file. In addition to the following steps, you can also use getRunsFilters to retrieve an existing filter as a sqlite table, and createRunsFilter to create a new filter with more properties (e.g. a description) or update the properties of an existing one. See \@ref(#appendixC) for more details. 
...
tbl.filter.1 = writeRunsFilter(sql.motus, "filtAmbigFalsePos", df = df.block.all, delete=TRUE)
...
tbl.alltags.sub <- left_join(tbl.alltags, tbl.filter.1, by = c("runID", "motusTagID")) %>%
