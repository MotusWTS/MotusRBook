...
There are three sources of 'error' that can result in false positive detections (tag detections appearing in your database that are incorrect). First, random radio noise ('static') can be detected and interpreted to be the transmission of a tag.  Second, despite our best efforts to avoid it, duplicate tags are sometimes transmitting in the network at the same time. When two tags are deployed at the same time that have the same Lotek ID code, burst interval, and nominal transmit frequency, it results in a situations where the detections may belong to either tag, but there is no information (other than contextual information) to separate them. Finally, 'aliased' tags can occur when two tags are transmitting at the same time and produce a signal that looks like a third tag that is not in fact present. Such aliased tags are most common at roosting sites or breeding colonies, where many tags are transmitting simultaneously. 

The key things to look for to determine if detections are false positives are:

1) Run length: run length ('runLen') is the number of tag bursts in a run, and a run is a group of consecutive detections of a tag detected on a receiver. In general, a detection with a run length of 2 has a high probability of being a false positive detection, arising from radio noise. With the exception of a few 'quiet' stations with little noise, we generally recommend that you filter out all detections with a run length of 2. However, because you will likely lose some true detections in the process, we also recommend that after a full analysis of your data, you return to these detections and examine them individuals, to determine (usually contextually) they can be considered real. 

2) Standard deviation of frequency offset: Standard deviation of frequency offset among pulses in a burst ('freqsd'; in KHz) may be used as a coarse and automatic data filter. , by removing any values larger that 0.1, which suggests a false detection. This filter applies to data collected by SensorGnome receivers only. As with run length, it is recommended that you identify the detections you would be removing and examine them to ensure you aren't removing an unwanted amounts of true detections.  
...
2) An animal with a properly activated tag may have never passed within range of a receiving station. Study designs that incorporate strategic placement of receivers to meet project goals can improve your odds of getting detections of your tags.
...
df.alltags %>%
  ...
  select(motusTagID) %>%
  distinct() %>%
  tally() # if you want to see the list of motusTagIDs, remove 'tally()'
...
For the purpose of this chapter, we simplify the sample data by subsetting to six motusTagIDs:  four tags (motusTagID = 22897, 22905, 23319, 22902) that were attached to Red Knots and are associated with ambiguous detections (ambigID = -114, -134, -171 respectively), and two tags (16038, 16047) that were attached to a Semipalmated Sandpiper and Red Knot, respectively, but not associated with ambiguous detections.

```{r subsetTags}

## generate a list of motusTagIDs that includes the motusTagIDs of duplicate tags 
## from other projects, i.e., associated with ambigIDs -114, -134, -171
ambigTags <- filter(df.clarify, ambigID %in% c(-114, -134, -171)) %>%
  select(ambigID, id1, id2, id3, id4, id5, id6) %>%
  gather(id, motusTagID, c(id1:id6), factor_key=TRUE) %>%
  filter(!is.na(motusTagID)) %>%
  pull(motusTagID) ## pull the variable motusTagID into a vector

## add in two additional tags -- 16038, 16047
tag.subset <- c(ambigTags, 16038, 16047)

## subset detections data to the tags of interest from our project 
## (this does not include detections of duplicate tags)
df.alltags.sub <- filter(df.alltags, motusTagID %in% tag.subset)

## get deployment records to tags of interest AND duplicate tags.
tbl.tagDeps <- tbl(sql.motus, "tagDeps")
df.tagDeps.sub <- tbl.tagDeps %>%
                    filter(tagID %in% tag.subset) %>%
                    select(deployID, tagID, projectID, fullID, tsStart, tsEnd, speciesID, 
                           latitude, longitude) %>%
                    collect() %>%
                    as.data.frame() %>%
                    mutate(
...
na.map <- subset(map_data(map="world2"), 
                 region %in% c("Canada", "USA", "Mexico", "lakes",
                               ...
                               "Puerto Rico", "British Virgin Islands", "Montserrat", 
                               "Dominica", "Saint Lucia", 
                               "Barbados", "Grenada", "Trinidad and Tobago", "Chile", 
...
```{r mapDetections1, warning = FALSE, message = FALSE}
...
```{r mapDetections3, message = FALSE, warning = FALSE}

df.alltags.path <- df.alltags.filt %>%
                    filter(tagProjID == proj.num) %>% # keep to tags registered to the sample project
                    arrange(motusTagID, ts) %>%       # order data by time stamp for each motus tag ID
                    mutate(date = as_date(ts)) %>%    # create date variable
                    group_by(motusTagID, date, recvDeployName, ambigID, tagDeployLon, 
                             tagDeployLat, recvLat, recvLon) %>%
                    summarize(max.runLen = max(runLen)) 
...
From these plots, we can make the following decisions:
...
There are no obvious false positive detections. The path follows an expected trajectory for a Red Knot departing on fall migration from the James Bay area.
...
It turns out that most of the ambiguous detections for this tag and tag 23319 were dropped as a result of runLen or freqsd. Using 'filter(df.tagDeps.sub, tagID == 22905|tagID == 23319)', we see that both tags were deployed by the James Bay Shorebird Project (Sample Project) within two weeks of each other, and from the same deployment location. As a result, all remaining ambiguities cannot be assigned with confidence to either tag. All ambiguous detections will be dropped for this tag and for tag 23319. Only non-ambiguous detections of tag 22905 between October 10 2016 when it was deployed, and October 15 2016 when the duplicate tag was deployed, will be useful for looking at local movement behaviour. 
...
ggplot(na.lakes, aes(long, lat))+ 
  ...
  theme_bw() + 
  geom_path(data = df.alltags.path, aes(recvLon, recvLat, group = motusTagID), colour = "grey20") + 
  geom_point(data = df.alltags.path, 
             aes(recvLon, recvLat, colour = !is.na(ambigID), shape = (max.runLen > 2)), size =1.5) +
  geom_point(data = df.alltags.path, 
...
## the various ambiguous tags come from.
...
There doesn't appear to be any issue with the remaining detections for this tag.
...
- 
=======
There are no unambiguous detections for this tag, so it can be dropped completely.
...
df.detectTrans <- as.data.frame(siteTrans(df.alltags.filt, latCoord = "recvLat", lonCoord = "recvLon")) 
## create new data.base consisting of transitions between consecutive sites for each tag
head(df.detectTrans)
