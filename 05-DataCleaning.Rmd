...
We match GPS locations to `hitID`s by `batchID` and according to one of several different time values, specified by the `by` argument. This can be the `closest` location in time, the `daily` median location, or the median location within `X` minutes of a `hitID` (here, `X` can be any number greater than zero and represents the size of the time block in minutes over which to calculate a median location).
