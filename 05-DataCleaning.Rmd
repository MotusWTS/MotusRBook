...
The goal of this chapter is to provide you with some tools to use to clean your data, and by the end of the chapter we will have a dataframe that contains the motusTagID, ambigID (where appropriate), and runID for detections associated with:
...
# Load allambigs table, which assigns an ambiguous ID (ambigID) to motus tag IDs associated with ambiguous detections

tbl.allambigs <- tbl(sql.motus, "allambigs")
df.allambigs <- tbl.allambigs %>%
                collect() %>%
                as.data.frame() 
...
If you do not have the 'rworldmap' package installed, then you must install before loading the package:

```{r installMapPackages, message = FALSE, warning = FALSE}

## install.packages("rworldmap")  # only install if not already installed, or if you want to update your package
require(rworldmap)
...
To map the paths, we set the x-axis and y-axis limits based on the location of receivers with detections. Depending on your data, these might need to be modified to encompass the deployment location of the tags, if tags were not deployed near towers with detections. We then use ggplot to plot the map and tag paths.
...
### Filter using plots of tag paths
...
- Using 'filter(df.alltags.sub, motusTagID == 16038, recvLon < -85)', we see that the detections near Lake Michigan occurred on August 21 and have a run length of only 2. Because we don't have a strong reason to trust these to be true detections, we can safely drop these observations from our data. 

To so so, we create a dataframe which contains the projectID, motusTagID, ambigID and runID of detections we would like to remove. We do not remove them from the data yet:

```{r filter1Tag1}

df.runsTag1 <- filter(df.alltags.sub, motusTagID == 16038, recvLon < -85) %>%
                  select(tagProjID, motusTagID, ambigID, runID) %>%
                  distinct()
...
- using 'detections in the Great Lakes region and in Nova Scotia have a run length of 2, and using filter(df.alltags.sub, motusTagID == 16047, (recvLon > -70|(recvLon < -75 & recvLat < 50))), we see that detections at the Great Lakes sites (recvDeployName 'Flamborough_Quarry' and 'Koffler') have freqsd > 0.2 in 3/4 cases. These detections also occur in winter, when Red Knots are on their South American wintering grounds. They are also outliers in relation to the other detections which have a run length of greater than 2, which show the Red Knot moving from the James Bay region to the east coast, i.e., along an expected migratory path for this species. Detections in the Great Lakes region and Nova Scotia can safely be considered false positives and dropped from the data. In this case, we will drop detections with longitude > -70, and with longitude < -75 where latitude is < 50:

```{r filter1Tag2}

df.runsTag2 <- filter(df.alltags.sub, motusTagID == 16047, (recvLon > -70|(recvLon < -75 & recvLat < 50))) %>%
                  select(tagProjID, motusTagID, ambigID, runID) %>%
                  distinct()
...
df.runsTag3 <- filter(df.alltags.sub, motusTagID == 22897, (!is.na(ambigID) & recvLon < -75 & recvLat > 25)|(runLen <= 2 & recvLon < -75 & recvLat > 25)) %>%
                  select(tagProjID, motusTagID, ambigID, runID) %>%
                  distinct()
...
- the ambiguous detections for this tag occur in four distinct regions. Using 'clarify(sql.motus)', the ambiguous detections could also belong to motusTagID 22778 from the RBrownAMWO project or motusTagID 24303 from the Neonics project. 
...
- using 'filter(df.tagDeps, tagID == 22778)', we see that the other duplicate tag was deployed on an American Woodcock on the east coast, at a latitude of 45.135 and longitude 67.293 (which is an error in the tag deployment metadata: the longitude should be negative!). Because shorebirds stage in the region where those ambiguities occurred, we will take a closer look (in a later section) at these observations before deciding how to treat them. 
...
df.runsTag4 <- filter(df.alltags.sub, motusTagID == 22902, ((!is.na(ambigID) & recvLon < -78 & recvLat > 41)|(runLen <= 2 & recvLon < -75 & recvLat > 41)|(recvLat < 25))) %>%
                  select(tagProjID, motusTagID, ambigID, runID) %>%
                  distinct()
...
- using 'filter(df.tagDeps.sub, tagID == 22905|tagID == 23319)', we see that this tag and tag 23318 were deployed by the James Bay Shorebird Project within two weeks of each other and from the same deployment location. As a result, most detections are ambiguous and cannot be assigned with confidence to either tag. All ambiguous detections will therefore unfortunately be dropped for this tag and tag 23319. Only non-ambiguous detections of tag 22905 between October 10 2016 when it was deployed, and October 15 2016 when tag 23319 were deployed, will be useful for looking at movement behaviour. 
...
df.runsTag5 <- filter(df.alltags.sub, motusTagID == 22905, (!is.na(ambigID))|(is.na(ambigID) & recvLon < -70)) %>%
                  select(tagProjID, motusTagID, ambigID, runID) %>%
                  distinct()
...
- detections in Saskatchewan, Canada are obvious outliers for this species and project, and using 'filter(df.alltags.sub, motusTagID == 23319, recvLon < -100)', we see that all detections have a run length of 2 and 3/4 have a freqsd > 0.1. They will therefore be dropped from our data (drop all detections with longitude < -100).

- using 'filter(df.alltags.sub, motusTagID == 23319, (recvLon > -100 & recvLon < -70 & is.na(ambigID)))', non-ambiguous detections in the Great Lakes region have a run length of 2 and 5/6 have freqsd > 0.1. These are likely false positives, and will be dropped from the data (drop all non-ambiguous detections with longitude < -70).

```{r filter1Tag6}

df.runsTag6 <- filter(df.alltags.sub, motusTagID == 23319, (!is.na(ambigID))|(recvLon < -70)) %>%
                  select(tagProjID, motusTagID, ambigID, runID) %>%
                  distinct()

```

#### Filter data based on tag paths

We create a dataframe which contains the motusTagID, ambigID, and runID for all detections we deem to not belong to our tags. As we go through this chapter, we will append any additional detections we decide are false positive:

```{r filterRuns1}

df.runs <- rbind(df.runsTag1, df.runsTag2, df.runsTag3, df.runsTag4, df.runsTag5, df.runsTag6)

```

And we use this list to filter our data:

```{r filter1 }

runs.list <- unique(df.runs$runID)

df.alltags.filt1 <- filter(df.alltags.sub, !(runID %in% runs.list))  # the ! means you keep runIDs that are NOT in the list
...
df.alltags.path <- df.alltags.filt1 %>%
                    filter(tagProjID == proj.num) %>% # keep to tags registered to the sample project
                    arrange(motusTagID, ts) %>%       # order data by time stamp for each motus tag ID
                    mutate(date = as_date(ts)) %>%    # create date variable
                    group_by(motusTagID, date, recvDepName, ambigID, tagDepLon, tagDepLat, recvLat, recvLon)%>%
...
ggplot(data = filter(df.alltags.filt1, is.na(ambigID)), # plot ambiguous detections first
            aes(ts, recvLat)) +
  geom_point(size = 1, shape = 1) +
  geom_point(data = filter(df.alltags.filt1, !is.na(ambigID)), aes(ts, recvLat), size = 1, shape = 2) + # add non-ambiguous detections
...
ggplot(data = filter(df.alltags.filt1, is.na(ambigID)), # plot ambiguous detections first
            aes(ts, recvLon)) +
  geom_point(size = 1, shape = 1) +
  geom_point(data = filter(df.alltags.filt1, !is.na(ambigID)), aes(ts, recvLon), size = 1, shape = 2) + # add non-ambiguous detections
...
#### Filter data based on plots of lat/lon by time

Again, we create a dataframe which contains the motusTagID, ambigID, and runID for all detections we deem to not belong to our tags, and append to the dataframe we created in the previous step (\@ref(filterPaths)).

Note that we are appending this new list of runIDs we want to delete to the previous list of runIDs:

```{r filterRuns1}

df.runs <- rbind(df.runs, df.runs2Tag1, df.runs2Tag3, df.runs2Tag4, df.runs2Tag6)
...
df.alltags.filt2 <- filter(df.alltags.filt1, !(runID %in% runs.list))  # the ! means you keep runIDs that are NOT in the list

```

Now re-run the above plot, and we now how just two individuals with movements between towers. Recall that the primary purpose of this data was to look at movement behaviour at the wintering site, so broad-scale movements are primarily bonus information:

```{r plotPaths2, message = FALSE, warning = FALSE}

df.alltags.path <- df.alltags.filt2 %>%
                    filter(tagProjID == proj.num) %>% # keep to tags registered to the sample project
                    arrange(motusTagID, ts) %>%       # order data by time stamp for each motus tag ID
                    mutate(date = as_date(ts)) %>%    # create date variable
                    group_by(motusTagID, date, recvDepName, ambigID, tagDepLon, tagDepLat, recvLat, recvLon)%>%
...
df.detectTrans <- as.data.frame(siteTrans(df.alltags.filt2)) ## create new data.base consisting of transitions between consecutive sites for each tag
head(df.detectTrans)
...
## Create a custom filter for your .motus file

Throughout this chapter, we used R code to filter out ambiguities and false positive detections that we determined did not belong to our tags. Instead of having to run through this code to filter the data each time you work with your .motus file, you can create custom filters (see \@ref(#appendixD)) that are attached to your user profile (motus login credentials), and which can be used to 1) assign probabilities to your detections, and 2) filter your detections based on those probabilities. For example, in this chapter we created the dataframe 'df.runs', which contains all of the motusTagIDs and associated runIDs for detections we would like to drop, i.e., we could use a filter to assign a probability of 0 to these detections. Those probabilities can then be used to filter your data each time your .motus file is loaded. We show you how to do this in three simple steps:

### 1. Create runID filter

Before we populate a filter with runIDs, we need to create a filter by assigning a name. The `createRunsFilter` will be used to do this. In the process, the function will check whether the name already exists. The function returns an integer filterID that will match the name of the new filter in the local database:

```{r createFilter}

createRunsFilter(sql.motus, filterName = "filtAmbigFalsePos", motusProjID = 176, descr = "filter to assign probability = 0 to false positive detections and ambiguities that belong to other tags (Chapter 5 in R Book)")

```
### 2. Create dataframe of runIDs, and add a probability of 0 to all detections 

We already have our df.runs dataframe, so all we need to do is add a probability of 0 to those detections:

```{r addProb0}

df.runs <- mutate(df.runs, probability = 0)

```

### 3. Write the runIDs to the filter

We then populate the filter with our list of runIDs using the `writeRunsFilter` function:

```{r writeFilter}

writeRunsFilter(sql.motus, "filtAmbigFalsePos", df = df.runs)

```

If you want to overwrite a previous version of this filter entirely, use the `delete = TRUE` statement:

```{r overwriteFilter}

writeRunsFilter(sql.motus, "filtAmbigFalsePos", df = df.runs, delete = TRUE)

```

Alternatively, if you want to append new records to the filter and leave previous records intact, use the `overwrite = FALSE` statement:

```{r overwriteFilter}

writeRunsFilter(sql.motus, "filtAmbigFalsePos", df = df.runs, overwrite = FALSE)

```

### 4. Filter detections with probability == 0

Finally, we can apply the filter to our data. Note that applying the filter simply assigns each detection with the probability you've assigned in the steps above. To also filter the data by those probabilities, you need to use the `p.min` statement. Here we drop anything with probability less than 0.5, although because we've assigned all runIDs that we want to drop a 0, there are only 0 and 1 probabilities in our data:

```{r overwriteFilter}

sql.filtered <- applyRunsFilter(sql.motus, "filtAmbigFalsePos", p.min = 0.5)
