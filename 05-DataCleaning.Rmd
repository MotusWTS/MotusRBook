...
For the purpose of this chapter, we simplify the sample dataframes by subsetting to six motusTagIDs:  four tags (motusTagID = 22897, 22905, 23319, 22902) are associated with ambiguous detections (ambigID = -114, -134, -171 respectively), and two tags (16038, 16047) do not have ambiguous detections.
...
df.alltags.path <- df.alltags.sub %>%
                    ...
                    summarize(max.sig = max(sig, na.rm = TRUE), max.runLen = max(runLen)) 
...
We see from the plots that 22905 and 23319 are duplicate tags, and share ambiguous detections, shown by the orange triangles in the above plot. A few other things pop out:

**1. motusTagID 16038:** 

- The detection near Lake Michigan is an unlikely location to detect a shorebird, and the run length of the detection is only 2. As we will see later, run lengths of 2 have a high probability of being false positive. We can therefore safely drop this observation from our data. To do so, we can drop all detections for this tag with longitude < -85 (see \@ref(FilterRunID) below).

**2. motusTagID 16047:** 

- detections in the great lakes region and in Nova Scotia have a run length of only two, and are outliers in relation to the other detections with a run length of greater than 2. These detections can safely be considered false positives and dropped from the data. In this case, we will drop detections with longitude > -70, and with longitude < -75 where latitude is < 50.

**3. motusTagID 22897:**

- this tag was released in Quebec, and the most likely migratory path of this shorebird was along the east coast of North America. 

- using 'clarify(sql.motus)', the ambiguous detections in the Great Lakes region could also belong to tag 24298 released by the Neonics project. Filtering the tag deployment metadata using 'filter(df.tagDeps, tagID == 24298)', we see that the duplicate tag was released on the north shore of Lake Erie. Those ambiguities, and non-ambiguous detections with a run length of 2 in that region can therefore safely be dropped from our data. In this case we will drop ambiguous detections and detections with a run length <= 2 that have longitude < -75 and latitude > 25. 

- The detections on the east coast and in South America are reasonable locations to detect shorebirds, so we keep those for now, despite those detections having a run length of 2, and thus a high probability of being false positive.
...
- we saw above that the Neonics project deployed tags on the north shore of Lake Erie in Ontario, so detections in that region can be safely removed from the sample data, i.e., drop detections with longitude < -75 and latitude > 41. 

- using 'filter(df.tagDeps, tagID == 22778)', we see that the other duplicate tag was deployed on an American Woodcock in the Bay of Fundy region, at a latitude of 45.135 and longitude 67.293 (which is an error: the longitude should be negative). Because this is a region where shorebirds stage, we will take a closer look at these observations before deciding how to treat them. 

- an ambiguous detection in the Delaware Bay region is a likely detection of our tag, so we retain it for now.

- a lone ambiguous detection in a forested region of interior Colombia has a run length of 2, and is likely a false positive detection of our shorebird. We can therefore drop detections with latitude < 25.

**5. motusTagID 22905:**

- this and tag 23318 were deployed at near simultaneous times, from the same site, and by the same project. As a result, most detections are ambiguous and can't be assigned with confidence to either tag. All ambiguous detections will therefore unfortunately be dropped for this tag. 

- non-ambiguous detections in the Great Lakes region have a run length of 2, and are in an unlikely location for our study species. These are likely false positives, and dropped from the data (drop all non-ambiguous detections with longitude < -70).
...
- detections in Saskatchewan, Canada are obvious outliers for this species and project, and can be assumed to be false positive and dropped from our data (drop all detections with longitude < -100).
