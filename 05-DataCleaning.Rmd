...
Second, despite our best efforts to avoid it, duplicate tags are sometimes transmitting in the network at the same time. When two tags are deployed at the same time that have the same Lotek ID code, burst interval, and nominal transmit frequency, it results in a situations where the detections may belong to either tag, but there is no information (other than contextual information) to separate them. We term these 'Ambiguous tags'. 
...
First, determine which project tags have detections, and determine how many are of run length = 22. Apart from the tag not being within range of a receiver station, there are several reasons why deployed tags might not be detected, including:
...
We would initiall plot a subset of tags by either latitude or longitude, to get an overview of where there might be issues. Here, to simplify the example, we plot only six tags. We avoid the ambiguous tags for now. 
...
Or, get a list of sites where the maximum detections were never greater than (say) 4, which may sometimes (but not always!) indicate they are simply false detections.
...
It is impossible here to go through every possible issue; users are encouraged to explore their data fully, and make reasoned decisions on which detections are unlikely or indeterminate. We will show you how to collect all of these in to a filter, which can then be routinely applied to your data prior to analysis. 
...
## create a new path data frame for plotting.
## use the function we created earlier to make a new 'path' data frame
...
Let's do the easy ones first. Ambiguous tag -337 is ambiguous only at the beginning of the deployment, but was detected at the exact latitude of deployment, and just before the non-ambiguous detections. So the issue is simply that the tail end of the previous deployment (by another project) overlaps with our deployment. We can claim these tags as our own, and remove the other ambiguous detections.
