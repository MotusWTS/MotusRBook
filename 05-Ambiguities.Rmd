...
The tag finder program only looks for tags in the raw detections data during their active lifetimes. Tag lifetimes are estimated based on tag metadata, including deployment date, tag model, and burst interval. If duplicate tags do not overlap in their active lifetimes, each detection is assigned unambiguously to the correct project and tag. This is the most common situation for duplicate tags in the Motus system.

In some cases, identical tags have been deployed and have overlapping lifetimes. When this happens, the detections are considered ambiguous and more than one than one tag (up to 6) could be the source of any given detection. 

Ambiguities can arise when two or more tags are transmitting the same signal and active at the same time, or they can arise due to errors in tag registration or deployment metadata.

If you find ambiguities in your data, first check that your tag metadata are complete, up-to-date, and accurate. If they are not, fix them, and .... ? 

```{r}
## I made this a chunk just to keep it separate

## PDT: (Now, wait for Denis to re-run the left_join, but presumably at some point just fix the metadata and re-run tagme)

## here is an example where we should just use require, and re-load these packages at the beginning. 
...
<<<<<<< HEAD
We will run through how to check for and deal with ambiguities using our sample dataset from the James Bay Shorebird Monitoring Project (see Section \@ref(sampleData)). 

PDT: Can we put this somewhere else? Such as in an introductory chapter?
We recommend that you run through the sample code in each chapter with the sample dataset **before** running through with your own data, because you may need to modify the code or the order of the steps we take in order to deal most effectively with your own ambiguities (every situation is different).
=======
We will continue using our sample dataset from the James Bay Shorebird Monitoring Project (see Section \@ref(sampleData)). In the previous chapter, we created a function to load several tables in the .motus file, and this subset of the data was output to a flat data file.  By loading the motusdata R package above, you now have direct access to the filtered data, which is named 'filteredAmbigs'. To look at the summary of this database, you can run the following code (not evaluated here):
...
Ambiguous detections are identified by using a unique negative value for the 'motusTagID' field of the 'allTags' view (see Section \@ref(databaseStructure)). All ambiguous detections can be viewed by using the clarify() function from the Motus R package. This function returns a table with ambigIDs that are equivalent to the negative motusTagIDs in the 'allTags' view, along with up to 6 true motusTagIDs that detections associated with the ambigID might represent, and a fullID that includes the project that each duplicate tag was deployed by:
...
... as discussed == this should go somewhere else.
...
filter(ambig.df, depLon > 0) %>% 
  select(tagProj, tagDeployID, speciesID, depLat, depLon) %>% 
  distinct()
...
## all of this should go somewhere else I think. For example, we should be creating any time variables right up front (and using the lubridate package). So move this section to the 'accessing data' chapter, or create another chapter ... a preparing data chapter. 
