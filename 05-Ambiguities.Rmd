...
1. Duplicate tags, which occur when two or more tags with the same Lotek ID code, burst interval, and nominal transmit frequency (i.e., the transmissions of tags are indistinguishable) are transmitting in the network at the same time, resulting in 'ambiguous' detections that could belong to either tag. 
...
There are a number of ways to do check for and filter false positives, and some of these are outlined in this chapter using the sample dataset from the James Bay Shorebird Project (see Section \@ref(sampleData)). Ultimately, it is up to the analyst to determine which detections are true and which should be removed.



*** Stick this somewhere else: If you find ambiguities in your data, first check that your tag metadata are complete, up-to-date, and accurate. If they are not, fix them online by logging into your project at <https:/motus.org>. Metadata in your .motus file can then be updated by running `r tagme(38, forceMeta = TRUE), eval = FALSE`. Note that it may take one or two days for changes made to metadata on the server to be reflected in your .motus file. 
...
In section \@ref(flatRDSfunction), we created a function to transform selected SQLITE tables in the .motus file to flat RDS files. For the purpose of this book, the resulting flat files for the sample dataset are included in the 'motusdata' package as RDA files. We load the detections data using:
...
## Check structure of detections data

As a first step, use summary() to look at the structure and range of values in each field:

```{r detectionsStructure}

summary(df.alltagswithambigs)
...
ambig.df <- filter(ambig.df, motusTagID %in% unique(allAmbigs$motusTagID))
