# Data Checking: Project Metadata {#checkMetadata}
...
To check the number of tags registered to your project, load the 'df.tags' RDS file for the James Bay Shorebird Monitoring Project, which was created using the 'createFlatFiles' function in Section \@ref(flatRDSfunction), and can be accessed here by loading the motusdata R package, which we did above. 
...
The following maps the location of tag deployments for project 38. If any deployment locations stand out as potentially incorrect, those should be subset, and metadata for those deployments checked and fixed (where appropriate) online at <https://motus.org>.
...
```
Note that there is a warning that 164 rows were removed from the map due to missing values. Exploring these using `r summary(df.tagDeps), eval = FALSE`, we see that 164 deployments are missing speciesID, latitude, and longitude. Comparing the tagIDs for these records in the 'df.tagDeps' file with the deployments of those tags online at <https://motus.org>, these tags were registered but were either not deployed or were used as test tags. Each deployment with missing metadata should be checked carefully, and missing metadata entered online for any tags that were deployed.

### Timing of tag deployments

Plotting tag deployment times can be a good way to check for odd or overlapping deployment dates when the same tag was deployed more than once.
...
3. number of project receiver deployments;
4. timinig of project receiver deployments;
5. location of network-wide and project receiver deployments;
...
To see which (in any) receiver deployments are registered to your project, first subset and summarize the data:
...
Plot of the location of Motus receivers, where receivers deployed by Project 38 are displayed in blue.
...
Not all receivers were deployed at the same time or during your study period. The above plot can be restricted to receivers that were active during a particular time by placing filters on date. For example, to restrict the plot to receivers that were active in 2016 (not evaluated here):
...
Maps provide better spatial context; the following steps plot the location of Motus receivers on a map of North America, with receivers deployed by project 38 displayed in red.
...
In this section, the focus is on metadata for receivers registered to a project, because only metadata for those receivers can be updated and fixed online by that project. These summaries can then be applied to a larger group of receivers, e.g., all receivers with detections or all receivers within certain geographic limits (with or without detections), depending on the goal of your analysis.
...
#### Receiver metadata from other projects

Many Motus users will want complete receiver and antenna metadata for any receiver with detections of their tags, including receivers deployed by other projects. To subset receiver metadata to all Motus receivers with detections of tags belonging to project 38:

```{r stationMetaWithDetect, eval = FALSE}
...
# receiver metadata for ALL Motus receiver deployments
df.recvDeps <- readRDS("./data/df.recvDeps.rds")

# select deployIDs that are within the detections database 'df.alltags'
df.stationDeps <- filter(df.recvDeps, deployID %in% unique(df.alltags$recvDeployID))

# join receiver and antenna metadata
df.stationDeps <- left_join(df.recvDeps, df.antDeps, by = "deployID")
...
You can then subset the data to look at particular variables, e.g., missing latitude (showing only first 6 rows):

```{r missingBearingDetectionsData}

head(filter(df.stationDeps, is.na(latitude)))
