...
Map showing the location of tag deployments for project 38. If you see any odd deployment locations, those should be subset, and metadata for those deployments fixed online at <https://motus.org>.
...
There are no missing start and end dates (tsStart/tsEnd), and deployment start dates range from `r min(year(df.tagDeps$tsStart)` to `r max(year(df.tagDeps$tsStart)`, which is reasonable for this project. However, there are `r length(df.tagDeps$speciesID[is.na(df.tagDeps$speciesID)])` records missing speciesID, latitude, and longitude. There is also at least one odd longitude of -980.69, which is an obvious typo. This should be fixed online at the source, but we fix it here as well, in both the tag deployment metadata file and the detections data:

```{r fixLongitude}

df.tagDeps <- mutate(df.tagDeps, longitude = replace(longitude, longitude == -980.6914, -80.6914))
df.alltagswithambigs <- mutate(df.alltagswithambigs, depLon = replace(depLon, depLon == -980.6914, -80.6914))
