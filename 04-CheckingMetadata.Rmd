...
To check the number of registered tags, load the 'df.tags' RDS file for the James Bay Shorebird Monitoring Project, which was created using the 'createFlatFiles' function in Section \@ref(flatRDSfunction), and can be accessed here by loading the motusdata R package, which we did aboce. The 'df.tags' file contains the characteristics of each registered tag, including a unique tagID and information on manufacturer, model, nominal and offset frequency, burst interval, and pulse length. We select the metadata specific to the James Bay Shorebird Monitoring Project, and ignore tag metadata associated with any duplicate tags:


```{r importTags, message = FALSE, warning = FALSE}

df.tags <- readRDS(df.tags)
df.tags <- df.tags %>% filter(projectID == 38)

```

The number of tags registered to the project is equivalent to the number of rows in the dataframe. There are 691 tags registered to the James Bay Shorebird Monitoring Project (recall we are using just a subset of this dataset):
...
```
If this number does not correspond with the number of tags that were registered with Motus, go through the list of tagIDs, determine which tags are missing, and ensure they are registered online at <https:motus.org/tag-registration>. If they are registered online, but not showing in your tag metadata, please contact motus@birdscanada.org.

### Number of registered tags that were deployed

The tag deployment table ('df.tagDeps') is required to check which of the registered tags have deployments. The 'df.tagDeps' database contains information on date, time, species, and location of tag deployment. We subset to our project ('38' for the sample data), and merge selected columns of the 'tagDeps' table with the 'tags' table to determine which registered tags have (or don't have) corresponding deployment information.
