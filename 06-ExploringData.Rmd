...
For Google Maps, we'll need to load ggmap and ggplot2 packages; see Chapter \@ref(loadingPackages) for instructions on installing the ggmap package if you have not already done so.
...
For mapping with outline maps, we'll need to load the rworldmap package; see Chapter \@ref(loadingPackages) for instructions on installing the package if you have not already done so.
...
```{r loadMaps.6, message = FALSE, warning = FALSE, echo = FALSE}

## get the lake data; adjust longitude
na.lakes <- fortify(maps::map(map="lakes", region=".", exact = FALSE, plot = FALSE, fill = TRUE)) %>% 
  mutate(long = long - 360)

## Include all of the Americas to begin
regions <- c("Canada", "USA", "Mexico", "lakes", "Belize", "Costa Rica", "Panama", 
...
na.map <- fortify(maps::map(map="world2", region=regions, exact = FALSE, plot = FALSE, fill = TRUE)) %>% 
  mutate(long = long - 360)

```

```{r loadMaps.6bDUP, eval = FALSE}
