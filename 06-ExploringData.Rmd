...
tbl.alltags.sub <- applyRunsFilter(sql.motus, "filtAmbigFalsePos", p.min = 0)
...
## if you are using the applyRunsFilter, you need to specify that you want the NA probability too
df.alltags.sub <- filter(df.alltags.sub, probability > 0 | is.na(probability))
...
df.alltags.sub <- mutate(df.alltags.sub, 
                         recvDepName = reorder(recvDeployName, recvLat)) 
