...
ggmap(gmap) +
  theme_bw() + 
  geom_point(data = df.tmp, aes(recvLon, recvLat), 
             shape = 21, colour = "black", fill = "yellow") +
  geom_path(data = df.tmp, 
...
siteOp <- with(df.recvDeps, lubridate::interval(tsStart, tsEnd))
...
# receivers with detections as yellow remember to register your google key if
# you haven't already!
# ggmap::register_google(key = "Your key here")
ggmap(gmap) +
  theme_bw() + 
