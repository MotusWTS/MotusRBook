...
If you do not get an error, but your 'ambig' database is empty: Congratulations, you have no ambiguities!!! Please count your lucky starts and skip to Chapter \@ref(checkMetadata). 
...
Next, we will visualize the time and location of detections relative to the time and location of tag deployments. Note that if you have a large number of ambiguities (say > 6) in your data, these plots will be messy. Instead, use the loop in 3) below to output ambigID-specific plots to PDF files. 
...
When we subset the data the the ambigID of interest, we will order the detections by timestamp.
...
filterProj38Ambigs <- function(df) {
  ...
  filt.data <- filter(df, ambigID != -134 & ambigID != -114 & motusTagID %in% allAmbigs$motusTagID[allAmbigs$projectID == 38]) %>% 
    collect() %>% 
    as.data.frame()
  
  
  filt.data <- mutate(filt.data, 
                      year = year(as_datetime(filt.data$ts)), 
                      date = date(as_datetime(filt.data$ts)),
                      recvLat = if_else(is.na(gpsLat), recvDeployLat, gpsLat), 
                      recvLon = if_else(is.na(gpsLon), recvDeployLon, gpsLon),
...
filt.data <- filterProj38Ambigs(df)
