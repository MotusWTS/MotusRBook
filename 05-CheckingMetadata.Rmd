...
In total, there have been `r nrow(recvDeps)` deployments since `r min(recvDeps$dateStart, na.rm = TRUE)'. A simple histogram will give us an idea of the type of receiver deployed, and how many deployments per receiver:
...
```
From the data summary we see there are `r nrow(projRecvs)` deployments registered to the project. Four deployments are missing latitude and longitude, and two deployments are missing end dates, which might mean they are still deployed (we will look at this in the next steps). 

If the number of receiver deployments does not correspond with the number of receivers you actually registered and deployed, you will need to go through the list of receiver deployIDs, determine which receivers are missing, and register them online at <https:motus.org>. Unless your receivers are registered with Motus, detections of your tags or the tags of others will not be processed and made available.

We can also look at the timing of deployments graphically; horizontal line(s) in the following plot show the deployment time(s) for every receiver (deviceID) registered to your project. Different deployments of the same device should not overlap in time:
...
ggplot(recvLong, aes(y = as.factor(deviceID), x = ts, colour = as.factor(deployID))) +
  geom_line() + theme_bw() +
  ylab("deviceID") + xlab("Year")

```
### Location of receiver deployments

We plot the location of project-specific and network-wide receivers using a simple plot of latitude against longitude, and then on a map of North America. First we add a variable to the database to distinguish receivers registered to your project vs. other projects.
...
ggplot(na.lakes, aes(long, lat))+ 
  ...
  scale_colour_manual(values = c("grey30", "red"), labels = "") + 
...
Users have access to the metadata for all receivers in the network. Here, we will check for the completeness and accuracy of metadata for receivers with detections of your tags. Depending on your project goals, the information you will need about receiver deployments might vary. For example, if you are interested in whether a tag was detected at a particular site, the number of antennas and their properties (type, height, orientation) might not matter. Please keep in mind, however, that as a collaborative network, other users might require more detail about your receiver and antenna deployments than you do; in the interest of benefiting the entire network, we encourage you to include as many details as possible in your receiver metadata. For this reason, we will check the completeness of all receiver and antenna metadata, keeping in mind that we can only modify the metadata for receivers registered to our own project.
