...
4. plot location of network-wide and project receiver deployments;
5. number of receivers with detections of your tags;
6. number of tags detected at your receivers;
7. completeness and accuracy of receiver metadata.
...
Each registered receiver might have been deployed numerous times. First we load the data from the .motus file that was imported earlier: 
...
recvDeps <- mutate(recvDeps, yrStart = year(as_datetime(recvDeps$tsStart, tz = "GMT")), 
                          dateStart = as.Date(as_datetime(recvDeps$tsStart, tz = "GMT")),
                          yrEnd = year(as_datetime(recvDeps$tsEnd, tz = "GMT")), 
                          dateEnd = as.Date(as_datetime(recvDeps$tsEnd, tz = "GMT")))
```

In total, there have been `r nrow(recvDeps)` deployments since `r min(recvDeps$dateStart, na.rm = TRUE)'. A simple histogram will give us an idea of how many times each receiver was deployed:
...
To see which of the deployments are registered to our project, we can simply subset and summarize the data:
...
```
Looking at the summary of the data, we see that there are `r nrow(projRecvs)` deployments registered to our project. Four deployments are missing latitude and longitude, and two deployments are missing end dates, which might mean they are still deployed (we will look at this in the next steps). 

If the number of deployments does not correspond with the number of receivers you registered and deployed, you will need to go through the list of receiver deployIDs, determine which receivers are missing, and register them online at <https:motus.org>. If receivers are not registered, detections of tags will not be available to you or other researchers with tags detected by you receiver.

### Location of receiver deployments

We plot the location of project-specific and network-wide receivers using a simple plot of latitude against longitude, and then on a map of North America. First we add a variable to the database to distinguish receivers registered to our project vs. other projects.

**Simple Plot:**

```{r simpleRecvPlot, message = FALSE, warning = FALSE}

recvDeps <- mutate(recvDeps, Project38 = if_else(projectID == 38, "Project 38", "Other Projects"))

# to show only receivers for your project, replace recvDeps with filter(recvDeps, projectID == 38)
ggplot(recvDeps, aes(y = latitude, x = longitude, colour = Project38)) +
  geom_point(pch = 1) +
  theme_bw() +
  ggtitle("Location of all Motus receiver deployments")+
  scale_colour_discrete(name = "")  # don't need legend name

```

Not all receivers are deployed at the same time, or during your study period. You can restrict the plot to receivers that were active during our study period by placing filters on date. For example, if you want to restrict the plot to receivers that were active in 2016, you can do the following:

```{r mapRecvLocationsbyTime, message = FALSE, warning = FALSE}

ggplot(filter(recvDeps, yrStart <= 2016 & yrEnd >= 2016), aes(y = latitude, x = longitude, colour = Project38)) +
  geom_point(pch = 1) +
  theme_bw() +
...
Because maps are more fun, we also plot the receivers on a map:
...
xmax <- -20 # restrict to the Americas
...
ggplot(na.lakes, aes(long, lat))+ 
  ...
  xlab("") + ylab("") + 
  theme_bw() + 
  geom_point(data = recvDeps, aes(longitude, latitude, colour = Project38), cex = 0.8, pch = 4)+
...
You can restrict the map to your own receivers by using the projRecvs dataframe created above, i.e., by subsetting recvDeps to the project of interest before setting the x-axis and y-axis limits. For the sake of demonstration, we restrict deployments to those that were active at some point in 2016.

```{r mapProjRecvs, message = FALSE, warning = FALSE}

# set limits to map based on locations of detections, ensuring they include the deployment locations
xmin <- min(projRecvs$longitude, na.rm = TRUE) - 0.5
xmax <- max(projRecvs$longitude, na.rm = TRUE) + 0.5
ymin <- min(projRecvs$latitude, na.rm = TRUE) - 0.1
ymax <- max(projRecvs$latitude, na.rm = TRUE) + 0.1
...
### Number of receivers with detections of your tags
