...
We will continue using our sample dataset from the James Bay Shorebird Monitoring Project (see Section \@ref(sampleData)). In the previous chapter on dealing with ambiguous detections (see Chapter \@ref(ambiguities)), we created a function at the end of the chapter to filter ambiguities that did not belong to our tags out of the .motus file, and output a filtered flat data file.  By loading the motusdata R package above, you now have direct access to the database 'filteredAmbigs'. To look at the summary of this database (not evaluated here):
...
If you would like to use your own data to go through this chapter, please go through Chapter \@ref(ambiguities) and deal with any ambiguities in your own data before going further. If there are no ambiguities in your data, you can import your .motus file and transform to a flat file as follows:
...
To check the number of registered tags, we need to load the 'tags' table from the .motus file, which includes the characteristics of each registered tag, including a unique tagID, and information on manufacturer, model, nominal and offset, burst interval and pulse length:
...
```
Two of the 691 registered tags were not deployed according to the metadata. Please check to ensure that this is correct. If the tags were indeed deployed, deployment metadata for these tags should be entered online at <https://motus.org>, otherwise the tagFinder will not search for these tags, and detections for these tags will be missing from your .motus database.
...
To list all tagIDs which have detections, you can run the following code; recall that we are using a sample of the James Bay Shorebird Monitoring Project data, so only those tags included in the sample dataset have detections:
