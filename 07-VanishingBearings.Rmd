...
p3 <- ggplot(subset(df.vanish, motusTagID == 16823 & ts > "2015-05-30 03:03:00" & ts < "2015-05-30 03:10:00"), aes(ts, sig, colour= as.factor(antBearing))) + 
  geom_point() +
  facet_grid(recvSiteName~.) +
  theme_bw()
...
df.vanish <- right_join(df.vanish, df.departTime, by = "motusTagID") %>% # in this case, right join should drop any individuals that don't have departure times in df.departTime
  filter(ts >= ts_depart,
