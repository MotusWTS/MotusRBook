...
4. **`allruns`**: similar to `alltags` but only provides 1 record per run (with tsBegin and tsEnd instead of ts, and is missing details about signal strengths of individual hits), instead of 1 record per hit. This is a good alternative to the alltags for many types of applications. This virtual table should be both much faster and smaller than alltags.
