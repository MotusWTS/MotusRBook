...
15. **`runs`**: detection data associated with a run (continuous detections of a unique tag on a given antenna or node).
...
4. **`allruns`**: similar to `alltags` but only provides 1 record per run (with tsBegin and tsEnd instead of ts, and is missing details about signal strengths of individual hits), instead of 1 record per hit. This is a good alternative to the alltags for many types of applications. This virtual table should be both much faster and smaller than alltags. We will soon provide examples for using this table.
...
However, if GPS data is required, users will either have to use the `alltagsGPS` view, or will have to add GPS values to a data subset (see Chapter 5 \@ref(addGPS)). 

```{r getAllTagsGPSTable, eval = TRUE}
tbl.alltagsGPS <- tbl(sql.motus, "alltagsGPS")
```

For many data applications, the *virtual* table `allruns` may be sufficient for your needs, and won't help greatly with speed and memory issues.  This table should be perfect for instance to track movements across receivers. Each record represent a run (unique series of hits on a single antenna or node), and provides the start and end time for each (tsBegin and tsEnd), instead of individual hits. It should generally safe to assume that an animal is present within the reception range of an antenna for the whole duration of a run, as longer interruptions in detections would result in new runs.

```{r getAllRunsTable, eval = TRUE}

# this retrieves the "allruns" table from the "sql.motus" SQLite file we read in earlier
tbl.allruns <- tbl(sql.motus, "allruns") # virtual table

```

We now have new `tbl.alltags` and `tbl.allruns` objects in R. The underlying structure of these tables is a list of length 2:
...
tbl.alltags <- tbl(sql.motus, "alltags") # virtual table
df.tag.12345 <- tbl(sql.motus, "alltags") %>% filter(motusTagID == 12345) %>% collect()
df.gps <- getGPS(sql.motus, data=df.tag.12345, keepAll=TRUE, by="closest")

# merge the data and gps dataframes
df.tag.12345 <- merge(df.tag.12345, df.gps, by=c("hitID")

```

If you are planning to use the allruns table, you should likely only need to rely on the deployment metadata that provides the receiver location. (TODO: provide a way to link runs and gps records.)
...
You can also do the same for the allruns view.

```{r df.allruns, eval = FALSE}
df.allruns <- tbl.allruns %>% 
  collect() %>% 
  as.data.frame()
```
