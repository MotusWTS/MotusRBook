...
df.flat <-  df %>% 
            collect() %>% 
            as.data.frame()      ## for all fields in the df
>>>>>>> 5bbd2fb7e072cccd10e298eb4e54c1b0db1653a3
...
df.flat.subset <- select(df, recv, ant, motusTagID) %>%
  distinct() %>% 
  collect() %>% 
  as.data.frame()  ## I include the brackets to make it clear it is a function/pdt     
=======
df.flat.subset <- select(df, recv, ant, motusTagID) %>%  ## grab a subset of fields
                  distinct() %>% 
                  collect() %>% 
                  as.data.frame()     
>>>>>>> 5bbd2fb7e072cccd10e298eb4e54c1b0db1653a3
...
<<<<<<< HEAD
df.flat.subset <- filter(df, motusTagID %in% c(16011, 23316)) %>% 
  collect() %>% 
  as.data.frame()    ## filter to only include detections of motus tag IDs 9939 and 25643
=======
...
<<<<<<< HEAD
df.flat.subset <- filter(df, speciesID == 4670) %>% 
  collect() %>% 
  as.data.frame()    ## filter by speciesID to only include detections of Swainson's Thrush

df.flat.subset <- filter(df, spEN == "Red Knot") %>% 
  collect() %>% 
  as.data.frame()    ## filter to only include detections of Red Knot (using English name)

```

You can also summarize your data before converting to a flat file. For example, to obtain a record of detections for each hour by receiver and antenna, you could do this:

```{r collectSum, eval = FALSE}

## Note: (PDT) ... do you show people how to summarize somewhere? e.g. something like
#hourly.sum.df <- df %>%
#  mutate(hour = 3600 * round(ts/3600, 0)) %>% 
#  group_by(motusTagID, recv, hour) %>%
#  summarize(
#    num.detect = length(hour), 
#    max.sig = max(sig)
#  )
=======
df.flat.subset <- filter(df, speciesID == 4670) %>% ## filter to include only Red Know by species ID
                  collect() %>% 
                  as.data.frame()    
df.flat.subset <- filter(df, spEN == "Red Knot") %>% ## filter to only include Red Knot by English name
                  collect() %>% 
                  as.data.frame()    

```

Using dplyr(), data can also be summarized before converting to a flat file. For example, to find the first hourly detection of each tag in each hour by receiver and antenna:

```{r collectSum, eval = FALSE}

hourly <- df %>% 
          mutate (hour = 3600 * round(ts / 3600, 0)) %>% 
          distinct (recv, ant, motusTagID, hour)
>>>>>>> 5bbd2fb7e072cccd10e298eb4e54c1b0db1653a3
...
A good workflow idea is to have a script that deals with all of your data issues, then saves a dataframe (or workspace) for re-use. If you do this, you can quickly start an analysis or visualization session from a known (and consistent) starting point. A good way to do this is by using an RDS file, which preserves all of the associate R data structures (such as time stamps).

```{r createRDS, eval = FALSE, message = FALSE, warning = FALSE}

# (pdt) use the dplyr functions throughout to be consistent?

## save an RDS file
saveRDS(df.flat, "./data/df.flat.RDS")  

## "df.flat"" is the name of the flat file within R that you want to save to an RDS file

## "./data/" is the the location where you would like the file stored. In this case it's in the data folder in the working directory

## "df.flat.RDS", is the name of the RDS file you are creating, this can be anything you like but should end with RDS (so you know what it is).
...
### Export your "flat" dataframe to a .rds file {#exportToRDS}

<<<<<<< HEAD
In some cases you may want to export your data to another program. The example below shows you how to save a flat file as a .csv file. 
...
## save a .csv file
write_csv(df.flat, "./data/sample_raw.csv")  ## "df.flat"" is the name of the flat file within R that you want to convert to a .csv

## "./data/" is the the location where you would like your .csv stored, in this case it's in the data folder in the working directory

## "sample_raw.csv", is the name of the .csv file you are creating, this can be anything you like but must end with .csv.
=======
The below example is how the flat file called 'df.flat' is saved as a .rds file called "sample_raw.rds" on your computer in the 'data' folder of your working directory. In this case the data are converted without subsetting.
...
saveRDS(object = df.flat, file = "./data/df.alltags.rds") # saves to the data folder of working directory

```

## Function to access and convert .motus files to flat files

For the purpose of this book, the following function creates flat .RDS files of the various tables that we will need from the .motus file. You can create a similar function for your own data, and add or remove data fields as necessary. 

```{r dataFunction}

# for your own data, replace with location of your 'project-XX.motus' data

createFlatFiles <- function(in.df =  system.file("extdata", "project-sample.motus", package = "motusdata")) {  
  ...
  ## extract and save alltagswithambigs view, selecting only parameters of interest (can be modified to suit needs)
  df <- tbl(t, "alltagswithambigs")
  df <- df %>% 
  ...
  df <- mutate(df, 
                  year = year(as_datetime(df$ts, tz = "GMT")), 
                  date = as.Date(as_datetime(df$ts, tz = "GMT")),
                  recvLat = if_else(is.na(gpsLat), recvDeployLat, gpsLat), 
                  recvLon = if_else(is.na(gpsLon), recvDeployLon, gpsLon))
  saveRDS(object = df, file = "./data/df.alltags")
  
  ## extract and save all metadata to flat files
  # tags registered
