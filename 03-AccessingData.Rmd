...
## Set the system environment time zone to GMT (to ensure that you are 
## always working in GMT)
Sys.setenv(TZ="GMT")
```
...
### logging out {#logout}
Once you are logged in under one account, you will not be able to access data from another account.  If you need to logout of your current account to access other data you can run the code below, for example if you need to logout of the sample project to run the tagme() function on your own project, or if you are already logged in to your project and wish to access the sample project.
```{r logout, eval = FALSE}
motusLogout()
```
...
proj.num <- 176 ## use the number associated with your project here

t <- tagme(projRecv = proj.num, update = TRUE, new = TRUE)                # for project tag database
t <- tagme(projRecv = "SG-123BBBK1234", update = TRUE, new = TRUE)  # for receiver tag database, note that there are no receivers registered to sample project 176, so this call will not find any data, you can however replace the receiver serial number with one registered to your project if you are logged in under your own credentials (ie. not motus.sample, see \@ref(logout))
```
...
t <- tagme(projRecv = proj.num, update = TRUE, new = TRUE, dir = "./data/") 
...
t <- tagme(projRecv = proj.num, new = FALSE, update = TRUE) ## if the file is in the local working directory, OR:
t <- tagme(projRecv = proj.num, new = FALSE, update = TRUE, dir = "./data/") ## use dir = to specify a directory
...
t <- tagme(projRecv = proj.num, forceMeta = TRUE)
...
vt <- tbl(t, "alltags") ## virtual table
```

The following table lists the variables available in the 'alltags' view, along with a description of each.
...
To convert the 'alltags' view or other table into a typical 'flat' format, i.e., with every record for each field filled in, use the collect() and as.data.frame() functions.  The output can then be further manipulated, or used to generate a RDS file of your data. 
...
write.csv(df, "./data/df.flat.CSV", row.names = FALSE)

```

### Access the book sample dataset

For the purposes of this book, the sample data from the James Bay Shorebird Project (see Section \@ref(sampleData)) has been downloaded and the .motus files made available through the motusdata R package. You can use the following code to load the sample data:
...
t <- src_sqlite(file.name)

```

### Convert .motus tables to 'flat' dataframes for use in the book {#flatRDSfunction}

For the purpose of this book, we  created a series of RDS files of the various tables that we will need from the .motus file. You can modify this script for your own data.

We include some basic data filters, create date and location variables, and select the variables that are likely to be of interest and use for most applications. These include:

1. creating 'lat' and 'lon' variables, which default to using the latitude and longitude derived from the GPS in each receiver, and in-fills missing GPS values with the latitude and longitude from the receiver deployment metadata entered online by the project that deployed the station.

2. creating an 'ambig' variable, which assigns a '1' when the detections could belong to two or more (up to 6) duplicate tags (see Chapter \@ref())

<<<<<<< HEAD
3. filtering the data by run length and standard deviation of frequency offset among pulses in a burst, which provides a simple way of excluding false positives. (We are working on more sophisticated methods that may help in the future). Generally, detections with runLen <= 2 have a high probability of being false positive, as do detections with freqSD > 0.1  kHz (see Chapter \@ref(dataCleaning)).
=======
# list of tables in your .motus file that you want to transform to flat files:
table.list <- c("alltagswithambigs", "tags", "tagDeps", "recvs", "recvDeps", "antDeps", "species", "allambigs")
>>>>>>> b3f3d640acbb929c31005d2ff20750cbc45dc76e

```{r dataFunction}
## full location of your .motus file. You would typically replace this code
## with t <- tagme(your project number)

in.df <- system.file("extdata", "project-sample.motus", package = "motusdata") 
t <- src_sqlite(in.df)

## create "alltagswithambigs"

vt <- tbl(t, "alltagswithambigs")

## we select here a minimal set of variables. There are many more, that may
## be useful for some purpose
df <- select(vt, 
             ts, sig, freqsd, motusTagID, 
             ambigID, ant, runLen, tagProjID, id, depLat, 
             depLon, recvDeployLat,recvDeployLon,
             recv, site, projID, proj, antType, antBearing, 
             antHeight, spEN, tagProj,  gpsLat, gpsLon) %>%
  collect() %>%
  as.data.frame() %>%
  mutate(
    ## fill in some missing lat/lons
    lat = if_else(is.na(gpsLat), recvDeployLat, gpsLat), 
    lon = if_else(is.na(gpsLon), recvDeployLon, gpsLon),
    
    ## create an ambiguous column, so detections are labelled as ambiguous or not
    ambig = if_else(is.na(ambigID), 0, 1),
    
    ## create a POSIX time stamp
    ts = as.POSIXct(ts, origin = "1970-01-01")) %>%
  select(-gpsLat, -gpsLon) %>%
  filter(runLen > 2, 
         freqsd < 0.12)

## Now save the flat file as an RDS file
saveRDS(df, "my.df.RDS")

## create an RDS file for the other table views we'll 
## use in the rest of the book
tbl(t, "tags") %>%
  collect() %>%
  select(tagID, projectID, mfgID, model) %>%
  as.data.frame() %>%
  saveRDS("df.tags.RDS")

tbl(t, "tagDeps") %>%
  collect() %>%
  select(tagID, status, tsStart, tsEnd, 
         projectID, speciesID, markerNumber, markerType, 
         latitude, longitude, fullID) %>%
  as.data.frame() %>%
  mutate(tsStart = as_datetime(tsStart), 
         tsEnd = as_datetime(tsEnd)) %>%
  saveRDS("df.tagDeps.RDS")
 
tbl(t, "recvDeps") %>%
  collect() %>%
  select(-macAddress, -elevation) %>%
  as.data.frame() %>%
  mutate(tsStart = as_datetime(tsStart), 
         tsEnd = as_datetime(tsEnd)) %>%
  saveRDS("df.recv.RDS")
