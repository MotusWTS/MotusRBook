...
This chapter will begin with an introduction into the data structure, followed by instructions on how to download and access the data.
...
- **new**: if set to "TRUE", it will create a new empty .motus file in your local directory. Do not use this parameter if you already have a .motus file. 
- **update**: Downloads all available data to your existing .motus file, must be set to "TRUE" on your first data download and any subsequent downloads if you wish to check for new data.
- **dir**: Your .motus data is automatically saved to your working directory, however you can specify a different location by using this parameter.
...
proj.num <- 176
# As this is the first time you're downloading data for project 176, we set new = TRUE, and update = TRUE
# this will create a .motus file in your current working directory as specified when you called getwd()
# and create an SQL obect in your R environment called "sql.motus"
...
# in this example we save to our data folder
sql.motus <- tagme(projRecv = proj.num, new = TRUE, update = TRUE, dir = "C:/Users/guest/Documents/data/")

```
The first time you run the tagme() function in an R session you'll be prompted to login;

```{r fig3.1, echo=FALSE, fig.align='center'}
knitr::include_graphics("images/SampleLogin.png")
```

For the purposes of the sample data we have created project 176 with the login name and password set to "motus.sample". To download data related to one of your projects, you simply need to use your Motus login/password, note that you will need to first logout if you're already logged in as the sample user, full details are in section \@ref(userAuthentication).

This will download a file to your working or specified directory called "project-176.motus". The download process should print its progress on the console; if you are not seeing it, try scrolling down your screen while tagme is running.
...
We could also download data by receiver through the same process as described above, this will provide you with all detections of any tags on the specified receiver. As there are no receivers registered to the sample project 176 **this call will not work**, you can however replace the below receiver serial number with one registered to your project if you're logged in under your own credentials (see section \@ref(logout)).
...
Users may wish to work directly with the SQLite file, however instructions to view the data as a typical "flat" dataframe within R (the format used throughout the rest of the book), and instructions to export it as a .csv file are in the sections below.

### Accessing downloaded detection data {#accessSQL}

Now that we've downloaded our data as an SQLite database and loaded it into an R object called "sql.motus"", we'll want to access the tables stored within the .motus file, detailed descriptions of all the tables are above in section \@ref(databaseStructure).
...
Some users may also want to export the flat dataframe into a csv for analysis in other programs, this can easily be done with the following code. Note that it **does not** preserve time stamps:
...
## Update and open an existing database {#tagmeUpdate}

As you or other users upload data to our server, you may have additional tag detections that weren't present in your initial data download. Since we use an SQLite database, you can easily update your existing .motus file with any newly available data rather than doing a complete new download. To open and update a detections database that already exists (has been downloaded previously), we use the tagme() function but set new = FALSE:
...
sql.motus <- tagme(projRecv = proj.num, update = FALSE) # use dir = to specify a directory
...
Tag and receiver metadata are automatically merged with tag detections when data are downloaded. However, if metadata has been updated since your initial download, you can force re-import of the metadata when updating a database by running: 
...
Throughout this book we will use sample data (see section \@ref(sampleData)) which has been assigned to project 176.  When accessing this data you will need to login using username and password 'motus.sample' in the R console when prompted by the tagme() function (see section \@ref(downloadData)). It will look like this:
...
## Summary: download, access, and export data
Throughout the book we will predominantly be working with the "alltags" table in flat format. Although described in detail above, here we include a quick summary of how you download, access, and convert the sample data for use in the rest of the book.
...
For your own data we suggest the following workflow; prepare a script that downloads/updates your data, filters out the necessary variables, and does any initial cleaning, and then save the resulting data as an RDS file as described in section \@ref(exportDetections). We suggest using RDS instead of CSV, because the RDS format preserves the underlying structure of the data (e.g. POSIX times stay as POSIX times). If you want to export your data to another program, then a CSV format might be preferred.  
