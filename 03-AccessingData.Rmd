...
sql.motus <- system.time(tagme(projRecv = proj.num, update = TRUE, new = TRUE))      # for project tag database
...
sql.motus <- tagme(projRecv = proj.num, new = FALSE, update = TRUE) ## if the file is in the local working directory, OR:
...
tbl.alltags <- tbl(sql.motus, "alltags")  # virtual table object of the alltags table in the sample.motus file
df.alltags <- tbl.alltags %>%
                collect() %>%
