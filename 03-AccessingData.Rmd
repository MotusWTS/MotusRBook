...
df.flat.subset <- filter(df, motusTagID %in% c(16011, 23316)) %>% collect %>% as.data.frame    ## filter to only include detections of motus tag IDs 9939 and 25643
...
df.flat.subset <- filter(df, speciesID == 4670) %>% collect %>% as.data.frame    ## filter to only include detections of Swainson's Thrush by species ID
df.flat.subset <- filter(df, spEN == "Red Knot") %>% collect %>% as.data.frame    ## filter to only include detections of Swainson's Thrush by English name
...
The below example is how you would save the flat file called "df.flat" as a .csv file called "sample_raw.csv" on your computer in the 'data' folder of your working directory. In this case we are simply converting the data without subsetting.

```{r createFlat, eval = FALSE, message = FALSE, warning = FALSE}
## to determine your working directory:
getwd()
## save a .csv file
write.csv(df.flat, "./data/sample_raw.csv")  ## "df.flat"" is the name of the flat file within R that you want to convert to a .csv
