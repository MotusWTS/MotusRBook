...
Throughout the book we will predominantly be working with the 'alltags' table in flat format. Although described in detail above, here we include a quick summary of how you download, access, and convert the sample data for use in the rest of the book. In this script, we include recommended modifications to the database (using the 'mutate' function) and filtering of unnecessary variables (using the 'select' function). More information is available on these and other data manipulation recommendations in section \@ref(loadDetectionsCleaning) in Chapter \@ref(dataCleaning).
