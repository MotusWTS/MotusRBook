...
In the event that your connection to the Motus server fails prior to the download completing (e.g., due to poor internet connection), use `r tagme(proj.num. update = TRUE), eval = FALSE` to continue the download from where it left off. 
...
sql.sample <- system.time(tagme(projRecv = proj.num, update = TRUE, new = TRUE))      # for project tag database
sql.sample <- tagme(projRecv = "SG-123BBBK1234", update = TRUE, new = TRUE)           # for receiver tag database
...
sql.sample <- tagme(projRecv = proj.num, update = TRUE, new = TRUE, dir = "./data/") 
...
sql.sample <- tagme(projRecv = proj.num, new = FALSE, update = TRUE) ## if the file is in the local working directory, OR:
sql.sample <- tagme(projRecv = proj.num, new = FALSE, update = TRUE, dir = "./data/") ## use dir = to specify a directory
...
sql.sample <- tagme(projRecv = proj.num, forceMeta = TRUE)
...
tbl.tagMeta <- tbl(sql.sample, "tagDeps")
...
tbl.alltags <- tbl(sql.sample, "alltags") ## virtual table
...
write.csv(df.alltags, "./data/df.alltags.CSV", row.names = FALSE)

```

## R object naming convention

Throughout this chapter and the rest of the book, we name R objects according to their structure and the source of the data contained in the object. So, SQLite objects will be prefixed with ".sql", virtual table objects will be prefixed with "tbl.", and dataframe objects will be prefixed with "df."; the rest of the name will be the name of the .motus table that the data originates from. For example, for the alltags table in the sample.motus file:

```{r namingConvention, eval = FALSE}

sql.sample <- tagme(176)                   # SQLite R object, which contains the .motus file
tbl.alltags <- tbl(sql.sample, "alltags")  # virtual table object of the alltags table in the sample.motus file
df.alltags <- tbl.alltags %>%
                collect() %>%
