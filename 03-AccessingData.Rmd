...
t <- tagme(projRecv = "SG-123BBBK1234", update = TRUE, new = TRUE)        # for receiver tag database
```
...
2. alltags: provides the full detections data for all duplicate tags associated with your data. Ambiguous detections are repeated for each motusTagID represented by each ambigID. 
...
The following table lists the variables available in the 'alltags' view [insert table with variable descriptions?]:
...
df.subset <- filter(vt, speciesEN == "Red Knot") %>%   
  collect() %>% 
  as.data.frame()  
