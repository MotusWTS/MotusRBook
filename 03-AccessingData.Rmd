...
t <- tagme(projRecv = 38, update = TRUE, new = TRUE)                 # for project tag database OR:
...
```{r tagme1b, eval = FALSE}

t <- tagme(projRecv = 38, update = TRUE, new = TRUE, dir = "C:/Data/") 
...
### Open a local tag database

To open a database that already exists (has been downloaded previously):

```{r tagme2, eval = FALSE}

t <- tagme(projRecv = 38) ## if the file is in the local working directory, OR:
t <- tagme(projRecv = 38, dir = "C:/Data/") ## use dir = to specify a directory

```

### Update and open a local tag database

To download any new data for a project or receiver that has already been downloaded to a local file in the current directory:
...
### Force an update/re-import of metadata
...
t <- tagme(projRecv = 38, forceMeta = TRUE)
...
tellme(projRecv = 38)                    ## If db is in the working directory
tellme(projRecv = 38, dir = "C/Data/")   ## To specify a different directory
...
tellme(projRecv = 38, new = TRUE)

```

### Access the book sample dataset

For the purposes of this book, the sample data from the James Bay Shorebird Project (see Section \@ref(sampleData)) has been downloaded and the .motus file made available through the motusdata R package. You can use the following code to load the sample data:
...
### Create function to output .motus tables to 'flat' RDS dataframes {#flatRDSfunction}
...
2. creating an 'ambig' variable, which assigns a '1' when the detections could belong to two or more (up to 6)
